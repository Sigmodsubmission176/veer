...
public class FilterPredicate {
    ...
    static {
        try {
            PredicateType = Class.forName("org.apache.kudu.client.KuduPredicate$PredicateType");
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        }
    }

    private final static Object[] enumElements = PredicateType.getEnumConstants();
    ...
    protected void mapKuduToTexera(Object type, byte[] lower, byte[] upper) {

            if(type.equals(enumElements[1])) {
                this.condition = ComparisonType.EQUAL_TO;
                this.value = Bytes.getString(lower);
            }
            else if(type.equals(enumElements[2])) {
            if (lower == null) {
                this.condition = ComparisonType.LESS_THAN;
                this.value = Bytes.getString(upper);
            } else if (upper == null) {
                this.condition = ComparisonType.GREATER_THAN_OR_EQUAL_TO;
                this.value = Bytes.getString(lower);
            }
        }
    }
